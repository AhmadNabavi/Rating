...
const RatingComponent: any = (props: any) => {
    ...
    const handleToggle = (_value: number, _inputParameterChanged: boolean) => {
        ...
        console.log("inputParameterChanged: " + _inputParameterChanged);

        if (!_inputParameterChanged) {
            for (let j = 0; j < _value; ++j)
                if (images[j].id % 3 === 2)
                    images[j].style.display = 'block';                  // show colored stars

            for (let i = _value; i < images.length; ++i)
                if (images[i].id % 3 === 1)
                    images[i].style.display = 'block';                  // show black&white stars
        }
        else {
            for (let j = 0; j < _value; ++j)
                if (images[j].id % 3 === 0)
                    images[j].style.display = 'block';                  // show selected stars
    ...
    const handleClick = () => {
        props.sendToOuput(Math.floor(((_rate + 1)) / 3));
    }

    React.useEffect(() => {
        handleToggle(props.rate * 3, true);
    }, [props]);
            ...
            <tbody>
                <tr>
                    <td><img id="1" src={star_blackwhite}
                        onMouseEnter={(e: any) => { handleCursor(e); handleToggle(parseInt(e.target.id), false); }}
                        onClick={handleClick}
                    /></td>
